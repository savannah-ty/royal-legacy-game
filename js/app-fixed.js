...
const RoyalLegacyApp = {
    ...
    showSettingsModal() {
        ...
        // 应用当前的字体和对比度设置到新创建的设置页面
        const currentState = StateManager.get();
        if (currentState.largeText) {
            // 应用大字体设置
            modal.querySelectorAll('p, span, h1, h2, h3, h4, h5, h6, .btn').forEach(elem => {
                elem.style.fontSize = '1.1em';
            });
            modal.querySelectorAll('h1, h2, h3, h4, h5, h6').forEach(elem => {
        ...
        if (currentState.highContrast) {
            // 应用高对比度设置
            modal.querySelectorAll('.glass-card').forEach(card => {
                card.style.background = 'rgba(26,26,46,0.9)';
                card.style.borderColor = 'rgba(212,175,55,0.6)';
            });
            modal.querySelectorAll('.btn').forEach(btn => {
                btn.style.borderWidth = '2px';
            });
            modal.querySelectorAll('p, span, div').forEach(elem => {
    ...
    toggleSetting(setting) {
        ...
        if (setting === 'largeText') {
            ...
            // 应用到所有文本元素，包括游戏页面
            document.querySelectorAll('p, span, h1, h2, h3, h4, h5, h6, .btn, #score, #steps, #gameGold, #gameStamina, .level-number, .level-star, #oneStarScore, #twoStarScore, #threeStarScore').forEach(elem => {
        ...
        if (setting === 'highContrast') {
            ...
            // 确保文字清晰可读，包括游戏页面的元素
            document.querySelectorAll('p, span, div, #score, #steps, #gameGold, #gameStamina, .level-number, .level-star, #oneStarScore, #twoStarScore, #threeStarScore').forEach(elem => {
        ...
        // 确保游戏页面也应用设置
        if (this.currentPage === 'gamePage' || this.currentPage === 'levelSelectPage') {
            this.updateGameStats();
            this.updateHomeStats();
        }
